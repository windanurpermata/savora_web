    ...
    <div class="max-w-3xl mx-auto">
        ...
        <form method="POST" action="{{ route('contributor.recipes.update', $resep->id) }}" enctype="multipart/form-data"
            ...
            <div class="bg-white rounded-xl border border-cokelat-100 p-5">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="category_id"
                                ...
                                @foreach ($kategoriList as $kat)
                                    <option value="{{ $kat->id }}"
                                        {{ old('category_id', $resep->category_id) == $kat->id ? 'selected' : '' }}>
                                        {{ $kat->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" name="waktu_memasak"
                                value="{{ old('waktu_memasak', $resep->waktu_memasak) }}" min="1" required
                                class="w-full bg-cokelat-50 border border-cokelat-200 rounded-lg px-4 py-2.5
                                   text-sm outline-none focus:border-cokelat-500 transition-colors">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="porsi" value="{{ old('porsi', $resep->porsi) }}" min="1"
                            required
                            class="w-full bg-cokelat-50 border border-cokelat-200 rounded-lg px-4 py-2.5
                               text-sm outline-none focus:border-cokelat-500 transition-colors">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" rows="4"
                            class="w-full bg-cokelat-50 border border-cokelat-200 rounded-lg px-4 py-2.5
                               text-sm outline-none focus:border-cokelat-500 transition-colors resize-none">{{ old('deskripsi', $resep->deskripsi) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl border border-cokelat-100 p-5">
                ...
                @if ($resep->gambar)
                    <div class="mb-3">
                        <p class="text-xs text-cokelat-400 mb-2">Foto saat ini:</p>
                ...
                <div class="border-2 border-dashed border-cokelat-200 rounded-xl p-6 text-center hover:border-cokelat-400 transition-colors cursor-pointer"
                    ...
                    <div id="preview-wrapper">
                        <svg class="w-8 h-8 text-cokelat-300 mx-auto mb-2" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                        </svg>
                        <p class="text-sm text-cokelat-400">Klik untuk ganti foto</p>
